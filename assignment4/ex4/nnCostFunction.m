...
J = sum(sum(-y_unrolled .* log(a3) - (1 - y_unrolled) .* log(1 - a3))) / m + ...
    lambda * (sum(sum(Theta1(:, 2:end) .^ 2)) + sum(sum(Theta2(:, 2:end) .^ 2))) / 2 / m;

delta3 = a3 - y_unrolled; % m x 10
delta2 = delta3 * Theta2(:, 2:end) .* sigmoidGradient(z2); % m x 25
